...
commentSchema.post("findOneAndDelete", async (doc: commentsType, next: () => void) => {
  ...
  try {
    ...
    const deleteReply = Reply.deleteMany({ _id: { $in: replies } }),
      //delete all like associated with that comment
      deleteCommentLikes = CommentLike.deleteMany({ Ref: doc._id }),
      //delete all reply like associated with that comment
      deleteReplyLikes = ReplyLike.deleteMany({ Ref: { $in: replies } });

    await Promise.allSettled([deleteReplyLikes, deleteReply, deleteCommentLikes]);
  } catch (e: any) {
    logg.warn(e.message);
  }
  return next();
